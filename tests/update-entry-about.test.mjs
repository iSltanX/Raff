...
test('About opens the update window when a newer version exists', async () => {
  const { dom, count } = mount('about.html', { status: 'available', version: '5.1.0' });
  await import('../src/js/about.js');
  await flush();
  dom.window.document.getElementById('update-btn').click();
  await flush();
